...
let path ~data_dir data =
  Filepath.(Data_dir.section_dir ~data_dir ~rel:false data.section / data.filename)
...
let download ~httpc ~data_dir data =
  ...
  let file_url = Filepath.(file + ".url") in
  ...
  let args =
    B0_http.Client.curl_fetch_args ~progress:true final_url Filepath.dash
  in
...
let get_or_download ?(available_log = Log.Info) ?httpc ~data_dir data =
  ...
  | Some httpc -> Ok httpc
  | None -> B0_http.Client.make ~progress:true ()
  in
