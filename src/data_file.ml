...
let download ~httpc ~data_dir data =
  ...
  let* () = Os.Cmd.run ~stdout Cmd.(tool "curl" %% args) in
...
let get ~data_dir data =
  let path = path ~data_dir data in
  let err e = match data.section with
  | Winvos ->
      Fmt.str "@[<v>%a@,Did you %a?@]"
        Fmt.lines e Fmt.code "telvm winvos update"
  | _ -> e
  in
...
let ensure ?(available_log = Log.Stdout) ~httpc ~data_dir data =
