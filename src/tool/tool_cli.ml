...
let data_dir_path_complete conf ~token =
  ...
  let files = List.map Filepath.to_string files in
...
let drive_device_and_data_dir_path_conv =
  let parser s =
    let* p = Filepath.of_string s in
    Qemu.drive_device_of_path p
  in
  let pp ppf (device, path) = match device with
  | None -> Filepath.pp ppf path
  | Some device ->
      Fmt.pf ppf "%a:%s" Filepath.pp path (Qemu.drive_device_to_string device)
  in
  let complete ctx ~token = match ctx with
  | None -> Ok Arg.Completion.[files]
  | Some ctx ->
...
let complete_inside_image ~image ~token =
  ...
  in
  let prefix = String.drop_first (* / *)  1 (Filepath.to_string token) in
  let paths = List.map Filepath.to_string paths in
  Ok (List.filter (String.starts_with ~prefix) paths)

let complete_image_file ~token =
  let* path = Filepath.of_string token in
  ...
  | Some (image, token) ->
      ...
      let comp rel =
        Arg.Completion.string (Filepath.to_string Filepath.(image // v rel))
      in
  ...
  | Some conf ->
      ...
      | Some (image, ptoken) ->
          ...
          let* timage = (* Complete w.r.t. to @ image path *)
            let suff = Filepath.to_string ptoken in
            let suff_len = String.length suff - 1 (* keep / *) in
            Filepath.of_string (String.drop_last suff_len token)
          in
          let comp rel =
            Arg.Completion.string (Filepath.to_string Filepath.(timage // v rel))
          in
          Ok (List.map comp rel_paths)
  in
  let completion = Arg.Completion.make ~context:config complete in
...
let arch_default_match_host ?opts ?doc () =
  ...
  match arch with None -> Os.Arch.get () | Some arch -> arch
...
let err_file_exists_use_force f =
  Fmt.str "@[<v>%a: File exists@,Use %a to overwrite@]"
    Filepath.pp f Fmt.code "--force"

let err_not_dir p = Fmt.str "%a: Not a directory" Filepath.pp p
let err_not_file_or_dir p = Fmt.str "%a: Not a file or directory" Filepath.pp p
let err_no_such_file_or_dir p =
  Fmt.str "%a: No such file or directory" Filepath.pp p
