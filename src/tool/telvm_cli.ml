...
let conf_envs =
  [ Cmd.Env.info "XDG_DATA_HOME" ~doc:"See option $(b,--data-dir).";
    Cmd.Env.info "XDG_STATE_HOME" ~doc:"See option $(b,--state-dir)." ]
...
let conf =
  ...
  and+ state_dir =
    ...
    Arg.(value & opt (some B0_std_cli.dirpath) None &
         info ["state-dir"] ~absent ~doc ~docs)
  in
  Telvm_conf.discover ~data_dir ~state_dir
...
let data_dir_path_conv =
  ...
  in
  let completion = Arg.Completion.make ~context:conf complete in
...
let complete_image_file ~token =
  ...
  let* split = Image.cut_file path in
  match split with
...
let image_path_conv =
  ...
  | Some conf ->
      ...
      let* split = Image.cut_file rpath in
      match split with
      | None ->
...
let image_partition_scheme_enum = Image.["mbr", Mbr; "gpt", Gpt]
let image_partition_scheme_conv =
  Arg.enum ~docv:"SCHEME" image_partition_scheme_enum

let image_partition_scheme =
  let doc =
    Fmt.str "$(docv) is the disk image partition scheme. Must be %s."
      (Arg.doc_alts_enum image_partition_scheme_enum)
  in
  Arg.(value & opt image_partition_scheme_conv Image.Gpt &
       info ["partition-scheme"] ~doc)

let image_file_system_enum = Image.["exfat", Exfat; "fat32", Fat32]
...
let image_file_system =
  ...
  in
  Arg.(value & opt image_file_system_conv Image.Exfat & info ["fs"] ~doc)
...
let mem_size_conv =
  ...
  | int, suff ->
      ...
      | Some int ->
          ...
          | "M" | "MO" | "MB" -> Ok (Fmt.str "%dM" int)
          | "G" | "GO" | "GB" -> Ok (Fmt.str "%dG" int)
...
let byte_size_conv =
  ...
  | int, suff ->
      ...
      | Some int ->
          match String.Ascii.uppercase suff with
          | "K" | "KO" | "KB" -> Ok (int * 1024)
          | "M" | "MO" | "MB" -> Ok (int * 1024 * 1024)
          | "G" | "GO" | "GB" -> Ok (int * 1024 * 1024 * 1024)
          | _ -> Fmt.error "Unknown size suffix %s" suff
  in
...
let list_cmd ?doc k =
  ...
  let+ conf and+ resolved in
  list ~kind:k ~conf ~resolved
