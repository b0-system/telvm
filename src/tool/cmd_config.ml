...
let pp_tool_version ppf tool =
  ...
  let field = Filepath.to_string (Cmd.get_tool tool |> Result.get_ok') in
...
let pp_qemu_version ppf guest_arch =
  ...
  let field = Filepath.to_string (Cmd.get_tool system |> Result.get_ok') in
  (Fmt.field field Fun.id pp_version) ppf version
...
let pp_conf ppf conf =
  ...
  let host_version = Os.Version.get () in
