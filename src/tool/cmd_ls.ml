...
let list_path ~dotfiles ~recurse ~rel ~follow_symlinks ~path =
  let output_path _stat _fname p () = print_endline (Filepath.to_string p) in
  let* is_dir = Os.Dir.exists path in
  if not is_dir
  then Ok (output_path () () (if rel then Filepath.basepath path else path) ())
...
let list_image_path
  ...
  let output_path _stat _fname p () =
    ...
    in
    print_endline (Filepath.to_string p)
  in
...
let ls ~config ~paths ~recurse ~follow_symlinks ~dotfiles ~full_path ~resolved =
  ...
  let list_path path =
    ...
    | None ->
        ...
        if not exists then Fmt.error "%a: No such path" Filepath.pp path else
  ...
  and+ paths =
    let doc =
      "Path to list. Repeatable. $(i,IMAGE) is the path to \
       ...
       file system."
    in
    Arg.(value & pos_all Tool_cli.image_path_conv [Filepath.v "."] &
         info [] ~doc)
  and+ recurse =
