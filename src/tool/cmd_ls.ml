...
let list_image_path
  ...
  let output_path _stat _fname p () =
    let p =
      if rel then p else
      Image.path_in_mount_to_image_path ~image ~mount_root p
    in
    print_endline (Fpath.to_string p)
  in
  let* mpath = Image.get_existing_path_in_mount ~image ~mount_root path in
...
let ls ~conf ~paths ~recurse ~follow_symlinks ~dotfiles ~full_path ~resolved =
...
let cmd =
  ...
  Cmd.make (Cmd.info "ls" ~doc ~man ~envs) @@
  ...
  and+ paths =
    let doc =
      "Path to list. Repeatable. $(i,IMAGE) is the path to \
       ...
       file system."
    in
    Arg.(value & pos_all Telvm_cli.image_path_conv [Fpath.v "."] &
         info [] ~doc)
  and+ recurse =
  ...
  and+ resolved = Telvm_cli.resolved in
  ls ~conf ~paths ~recurse ~follow_symlinks ~dotfiles ~full_path ~resolved
