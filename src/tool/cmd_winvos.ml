...
let create
    ~conf ~dry_run ~accel ~mem_size ~with_shutdown ~plan ~imager ~imager_arch
...
let bootstrap
    ~conf ~accel ~mem_size ~build_script_file ~imager_arch ~force ~dst
  =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
...
let bootstrap =
  ...
  and+ dst =
    ...
    Arg.(value & pos 0 (some ~none Telvm_cli.data_dir_path_conv) None &
         info [] ~doc ~docv)
  in
  bootstrap ~conf ~accel ~mem_size ~build_script_file ~imager_arch ~force ~dst
...
let create =
  ...
  and+ with_shutdown = Telvm_cli.with_shutdown in
  create
    ~conf ~accel ~mem_size ~dry_run ~plan ~imager ~imager_arch ~image_arch
