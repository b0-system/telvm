...
let make_plan_image ~data_dir ~script_src ~image_arch:arch ~dst =
  ...
  let* () =
    let script_file = Filepath.(mount_root / "create-image.cmd") in
    Os.File.write ~force:false ~make_path:false script_file script_src
  in
  (*
  let* () =
    let dst = Filepath.(mount_root / "telvm") in
    Os.Dir.copy ~make_path (Filepath.v "telvm") ~dst
  in *)
  let* () =
    let* powershell_zip =
    ...
    in
    Log.stdout (fun m ->
        m "Extracting %s in plan image" (Filepath.basename powershell_zip));
    let dst = Filepath.(mount_root / "powershell") in
...
let create
  ...
  let* () =
    Log.stdout
      (fun m ->
         m "Extracting ValidationOS.vhdx from %s" (Filepath.basename winvos_iso));
  ...
  let* () =
    ...
    let build_log = Filepath.(mount_root / "log.txt") in
...
let log ~config ~no_pager ~op =
  ...
  let warn_no_log log = Log.warn (fun m -> m "No %a file." Filepath.pp log) in
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  ...
  | `Output_path ->
      Ok (print_endline (Filepath.to_string log))
