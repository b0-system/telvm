...
let make_plan_image ~data_dir ~script_src ~image_arch:arch ~dst =
  ...
  in *)
  let* () =
    let* powershell_zip = Winvos.get_powershell_zip_file ~data_dir ~arch in
    Log.stdout (fun m ->
...
let create
    ~conf ~dry_run ~mem_size ~with_shutdown ~plan ~imager ~imager_arch
  ...
  let* image =
    ...
    if exists && not force
    then Error (Telvm_cli.err_file_exists_use_force image)
    else Ok image
  in
  ...
  Adhoc.with_tmp_file ~ext:".img" @@ fun plan_image ->
  let* () = make_plan_image ~data_dir ~script_src ~image_arch ~dst:plan_image in
  let* virtio_win_iso = Data_file.get ~data_dir Winvos.virtio_win_iso in
  let* winvos_iso = Winvos.get_iso_file ~data_dir ~arch:image_arch in
  let* () =
    Log.stdout
      (fun m ->
         m "Extracting ValidationOS.vhdx from %s" (Fpath.basename winvos_iso));
    Image.with_mount ~image:winvos_iso @@ fun ~mount_root:winvos_mount ->
  ...
  let* qemu =
    Qemu.cmd
      ~guest_arch:imager_arch ~mem_size ~drives ~use_virtio:true
      ~use_usb_input:true ~graphic:false ~use_ramfb:false
  in
  if dry_run
  then (Log.stdout (fun m -> m "%a" Adhoc.pp_dry_run qemu); Ok 0) else
  with_log_file (Telvm_conf.winvos_create_log_file conf) @@ fun log_fd ->
  log_start log_fd;
  ...
  in
  let* () =
    Image.with_mount ~image:plan_image @@ fun ~mount_root ->
...
let bootstrap ~conf ~mem_size ~build_script_file ~imager_arch ~force ~dst =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
  ...
  let* () =
    Winvos.Imager.make
      ~data_dir ~mem_size ~bootstrap_arch ~build_script_src ~imager_arch ~force
      ~make_path:true ~dst
  in
  Ok 0

let package_contents ~conf ~arch ~names ~add_matching_wow64 =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
  let* () = Winvos.Pkg.contents ~data_dir ~arch ~names ~add_matching_wow64 in
  Ok 0

let package_list ~conf ~arch ~names ~add_matching_wow64 =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
  let* () =
...
let log ~conf ~no_pager ~op =
  ...
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let log = Telvm_conf.winvos_create_log_file conf in
...
let plan ~conf ~arch ~which ~with_shutdown ~script =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
...
let reset ~conf =
  Log.if_error ~use:Cmdliner.Cmd.Exit.some_error @@
  let data_dir = Telvm_conf.data_dir conf in
  let winvos_dir = Data_dir.section_dir ~data_dir Winvos ~rel:false in
  Log.info (fun m -> m "Deleting %a" Fpath.pp winvos_dir);
  let* _existed = Os.Dir.delete ~recurse:true winvos_dir in
  Ok 0

let update ~conf ~mode ~archs =
...
let archs =
  ...
  Arg.(value & opt_all Telvm_cli.arch_conv Winvos.archs &
...
let plan =
  ...
  in
  Cmd.make (Cmd.info "plan" ~doc ~man ~envs) @@
  ...
  plan ~conf ~arch ~which ~script ~with_shutdown
...
let bootstrap =
  ...
  in
  Cmd.make (Cmd.info "bootstrap" ~doc ~man ~envs) @@
  let+ conf = Telvm_cli.conf
  and+ mem_size = Telvm_cli.mem_size
  and+ build_script_file =
    ...
    Arg.(value & opt (some Telvm_cli.data_dir_path_conv) None &
         info ["script"] ~doc ~docv:"[@]SCRIPT.cmd" ~absent)
  and+ imager_arch =
    let doc = "$(docv) is the architecture of the imager OS." in
    let opts = ["imager-arch"] in
    Telvm_cli.arch_default_match_host ~doc ~opts ()
  and+ force = Telvm_cli.force
  and+ dst =
    let doc = "The destination file to write the image to." in
    let none = "@winvos/imager-$ARCH.img" in
    let docv = "[@]IMAGER.img" in
    Arg.(value & pos 0 (some ~none Telvm_cli.data_dir_path_conv) None &
         info [] ~doc ~docv)
  in
  bootstrap ~conf ~mem_size ~build_script_file ~imager_arch ~force ~dst
...
let create =
  ...
  and+ imager =
    let doc =
      "$(docv) is the imager to use. If no imager is specified and \
       no imager is found in $(b,@winvos) the bootstrap procedure is \
       performed as if $(cmd.parent) $(b,bootstrap) is invoked."
    in
    let none = "@winvos/imager-$HOST_ARCH.img" in
    let docv = "[@]IMAGER.img" in
    Arg.(value & opt (some ~none Telvm_cli.data_dir_path_conv) None &
  ...
  and+ imager_arch =
    ...
    in
    let opts = ["imager-arch"] in
    Telvm_cli.arch ~doc ~opts ~absent ()
  and+ image_arch =
    let absent = "See below" in
    let doc =
  ...
  and+ with_shutdown = Telvm_cli.with_shutdown in
  create
    ~conf ~mem_size ~dry_run ~plan ~imager ~imager_arch ~image_arch ~force
    ~image ~with_shutdown
...
let log =
  ...
  in
  let envs = Telvm_cli.conf_envs @ B0_pager.Env.infos in
...
let package =
  ...
  let arch = Telvm_cli.arch_default_match_host () in
  let pkg_name =
    ...
    in
    let context = Term.(const (fun x y -> x, y) $ conf $ arch) in
  ...
  let list =
    ...
    in
    Cmd.make (Cmd.info "list" ~doc ~man ~envs) @@
    let+ conf and+ arch and+ names and+ add_matching_wow64 in
    package_list ~conf ~arch ~names ~add_matching_wow64
  in
  let contents =
    ...
    in
    Cmd.make (Cmd.info "contents" ~doc ~man ~envs) @@
    let+ conf and+ arch and+ names and+ add_matching_wow64 in
    package_contents ~conf ~arch ~names ~add_matching_wow64
  in
  Cmd.group (Cmd.info "package" ~doc ~man ~envs) @@
  [list; contents]
...
let update =
  ...
  let man = [
    ...
    `P "$(cmd) updates, if needed, the WinVOS iso files and other support \
        files." ]
  in
  Cmd.make (Cmd.info "update" ~doc ~man ~envs) @@
  let+ conf = Telvm_cli.conf and+ archs
  and+ mode =
    let check =
  ...
  in
  update ~conf ~mode ~archs

let reset =
  let doc = "Delete all WinVOS data" in
  let man = [
    `S Manpage.s_description;
    `P "$(cmd) deletes the $(b,@winvos) directory"]
  in
  Cmd.make (Cmd.info "reset" ~doc ~man ~envs) @@
  let+ conf = Telvm_cli.conf in
  reset ~conf
...
let version =
  ...
  in
  Cmd.make (Cmd.info "version" ~doc ~man ~envs) @@
  let+ conf = Telvm_cli.conf and+ archs
  and+ mode =
    let check =
      Arg.info ["c"; "check"] ~doc:
...
let cmd =
  ...
  in
  Cmd.group (Cmd.info "winvos" ~doc ~man ~envs) @@
