...
| Some _ ->
    Error (Telvm_cli.err_not_file_or_dir dst)
...
let rec cp_src_dir
  ...
  | Some { Unix.st_kind = S_REG } ->
      Error (Telvm_cli.err_not_dir dst)
  | Some _ ->
      Error (Telvm_cli.err_not_file_or_dir dst)
...
and cp_with_src
  ...
  | Some _ -> Error (Telvm_cli.err_not_file_or_dir src)
  | None -> Error (Telvm_cli.err_no_such_file_or_dir src)
...
let cp_src
  ...
  let* split = Image.cut_file src in
  ...
  | Some (src_image, src) ->
      let src_image_with_mount ~dst_mount ~src_image f =
        ...
        | None -> Image.with_mount ~image:src_image f
        | Some (dst_image_realpath, dst_mount) ->
            ...
            else Image.with_mount ~image:src_image f
      in
      ...
      let* src =
        ...
        Image.get_existing_path_in_mount ~image ~mount_root src
      in
      cp_with_src
        ~force ~recurse ~follow_symlinks ~src ~src_base ~content ~dst_stat ~dst
  in
...
let cp_with_dst
  ...
  | srcs ->
      ...
      | _ -> Error (Telvm_cli.err_not_dir dst)

let cp_cmd ~conf ~srcs ~content ~dst ~force ~recurse ~follow_symlinks =
  ...
  let* split = Image.cut_file dst in
  let* () = match split with
  | None ->
      cp_with_dst
  ...
  | Some (image, dst) ->
      ...
      let* dst = Image.get_syntactic_path_in_mount ~image ~mount_root dst in
...
let cmd =
  ...
  Cmd.make (Cmd.info "cp" ~doc ~man ~envs) @@
  ...
  and+ srcs =
    ...
    Arg.(non_empty & pos_left ~rev:true 0 Telvm_cli.image_path_conv [] &
         info [] ~doc ~docv)
  and+ dst =
    let doc =
    ...
    Arg.(required & pos ~rev:true 0 (some Telvm_cli.image_path_conv) None &
         info [] ~doc ~docv)
  and+ force =
  ...
  in
  cp_cmd ~conf ~srcs ~content ~dst ~force ~recurse ~follow_symlinks
