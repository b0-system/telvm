...
let err_is_dir p = Fmt.str "%a is a directory (not copied)" Filepath.pp p
...
| Some { Unix.st_kind = S_DIR } ->
    let dst = Filepath.(dst / src_base) in
...
let rec cp_src_dir
  ...
  | Some { Unix.st_kind = S_DIR } ->
      if not content then
        ...
        Ok (Log.warn (fun m -> m "Skipping directory %a" Filepath.pp src))
      else
      let cp src =
        let src_base = Filepath.basename src in
...
and cp_with_src
  ...
  =
  let* src_stat =
    if Filepath.is_dash src then Ok None else Os.Path.exists_stat src
  in
  match src_stat with
  | Some { Unix.st_kind = S_REG } ->
      cp_src_file ~force ~src ~src_base ~dst_stat ~dst
  | None when Filepath.is_dash src ->
      cp_src_file ~force ~src ~src_base ~dst_stat ~dst
  | Some { Unix.st_kind = S_DIR } ->
  ...
  | Some (src_image, src) ->
      let src_image_with_mount ~dst_mount ~src_image f =
        ...
        | Some (dst_image_realpath, dst_mount) ->
            ...
            if Filepath.equal dst_image_realpath src_image_realpath
      ...
      let src_base =
        if Filepath.is_root src then Filepath.basename src_image else
...
let cp_with_dst
  ...
  let* dst_stat =
    if Filepath.is_dash dst then Ok None else Os.Path.exists_stat dst
  in
  match srcs with
  | [] -> assert false
  | [src] ->
