...
let run
    ~conf ~guest_arch ~mem_size ~dry_run ~drives ~graphic
  ...
  let* cmd =
    ...
    Qemu.cmd
      ~guest_arch ~mem_size ~drives ~graphic
      ~use_ramfb ~use_virtio ~use_usb_input
  in
  let* () =
    if dry_run
    ...
    let* () =
      List.iter_stop_on_error (fun (_, p) -> Os.File.must_exist p) drives
    in
    Os.Cmd.run cmd
  in
  Ok 0
...
let cmd =
  ...
  and+ drives =
    let doc =
      "$(docv) is a disk image exposed as a disk drive in the run. \
       ...
       $(i,DEVICE) specifies the kind of drive. Must be one of \
       ...
       devices."
    in
    Arg.(non_empty & pos_all Telvm_cli.drive_device_and_data_dir_path_conv [] &
         info [] ~doc)
  and+ no_virtio =
    let doc =
  ...
  in
  run ~conf ~guest_arch ~drives ~mem_size ~dry_run ~graphic
