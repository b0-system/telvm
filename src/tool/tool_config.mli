...
val make : data_dir:Filepath.t -> state_dir:Filepath.t -> t
(** [make] is a configuration with given parameters. See accessors
    for semantics. *)

val discover :
  data_dir:Filepath.t option -> state_dir:Filepath.t option ->
  (t, string) result
...
val state_dir : t -> Filepath.t
(** [state_dir conf] is the state directory. May not exist. *)

val winvos_create_log_file : t -> Filepath.t
