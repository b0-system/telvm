...
let with_cwd ~conf ~cwd ~tool ~tool_args =
  ...
  let* split = Image.cut_file cwd in
  ...
  | Some (image, cwd) ->
      ...
      let* cwd = Image.get_existing_path_in_mount ~image ~mount_root cwd in
      Os.Cmd.run ~cwd cmd
  in
  Ok 0
...
let cmd =
  ...
  Cmd.make (Cmd.info "with-cwd" ~doc ~man ~envs) @@
  ...
  and+ cwd =
    let doc =
      "$(docv) is the current working directory to set. $(i,IMAGE) is the \
       ...
       file system."
    in
    Arg.(required & pos 0 (some Telvm_cli.image_path_conv) None & info [] ~doc)
  and+ tool =
  ...
  in
  with_cwd ~conf ~cwd ~tool ~tool_args
