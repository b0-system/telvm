...
let cmd =
  ...
  Cmd.make (Cmd.info "rm" ~doc ~man ~envs) @@
  ...
  and+ paths =
    let doc =
      "Path to delete. Repeatable. $(i,IMAGE) is the path to \
       ...
       file system."
    in
    Arg.(non_empty & pos_all Telvm_cli.image_path_conv [] & info [] ~doc)
  and+ recurse =
    let doc = "Delete paths recursively." in
    Arg.(value & flag & info ["r"; "R"; "recurse"] ~doc)
  in
  rm ~conf ~paths ~recurse
