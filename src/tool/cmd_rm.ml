...
let warn_no_path p =
  Log.warn (fun m -> m "%a: No such file or directory" Filepath.pp p)

let warn_image_empty ~image =
  Log.warn (fun m -> m "%a: Image is empty" Filepath.pp image)

let err_no_recurse ~image =
  Fmt.str "%a: Use option %a to delete the image contents"
    Filepath.pp image Fmt.code "-r"
...
let rm_path ~data_dir ~recurse path =
  ...
  | Some (image, path) ->
      ...
      in
      if Filepath.equal path mount_root
