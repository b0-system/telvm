...
let err_image_root p = Fmt.str "%a: Cannot move image root" Filepath.pp p
...
let rec find_dst_dev dst =
  if Filepath.is_root dst then Error "Could not find destination device" else
  match Os.Path.exists_stat dst with
  | Error _ as e -> e
  | Ok None -> find_dst_dev (Filepath.parent dst)
...
let mv_src ~data_dir ~force ~src ~dst_mount ~dst_stat ~dst =
  ...
  | Some (src_image, src) ->
      ...
      let* src_base =
        if Filepath.is_root src
        then Error (err_image_root Filepath.(src_image // src))
        else Ok (Filepath.basename src)
      in
      let* src =
