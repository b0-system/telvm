...
val accel :
  host_os:Os.Name.t -> host_arch:Os.Arch.t -> guest_arch:Os.Arch.t -> string
...
val auto_base_cmd :
  ...
  smp:string option -> mem:string option -> (Cmd.t, string) result

(** {1:bootloader Bootloader parameters} *)

val uefi_code :
  host_os:Os.Name.t -> guest_arch:Os.Arch.t -> (Fpath.t, string) result
(** [uefi_code] determines the file where the UEFI blob stored. *)

val uefi_drive : uefi_code:Fpath.t -> Cmd.t
...
val choose_drive_devices :
  use_virtio:bool -> (drive_device option * Fpath.t) list -> drive list
(** [choose_drive_devices ~use_virtio] has the logic to default drive
    ...
    special case for the first drive. *)
