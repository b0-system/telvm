...
val uefi_code :
  host_os:Os.Name.t -> guest_arch:Os.Arch.t -> (Filepath.t * string, string) result
(** [uefi_code] determines the file where the UEFI blob stored and its
    format *)

val uefi_drive : uefi_code:(Filepath.t * string) -> Cmd.t
...
val drive_device_of_path :
  Filepath.t -> (drive_device option * Filepath.t, string) result
...
val choose_drive_devices :
  use_virtio:bool -> (drive_device option * Filepath.t) list ->
