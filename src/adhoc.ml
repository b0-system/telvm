...
let print_paths paths =
  if paths = [] then () else Fmt.pr "@[<v>%a@]@." Fmt.(list Filepath.pp) paths
...
let scrape_arch_of_filepath file =
  ...
  let p = String.map normalize (Filepath.to_string file) in
...
| None ->
    ...
    | Some arch -> arch
    | None ->
        let host = Os.Arch.get () in
        Log.warn
          (fun m -> m "%a: no architecture found using %a"
              Filepath.pp file (Fmt.st' [`Bold] Os.Arch.pp) host);
        host
