...
val make :
  ...
  Filepath.t -> (unit, string) result
(** [name] maybe truncated depending on [format]. *)

val with_mount' :
  image:Filepath.t -> (mount_root:Filepath.t -> 'a) -> ('a, string) result
...
val with_mount :
  image:Filepath.t -> (mount_root:Filepath.t -> ('a, string) result) ->
  ('a, string) result
...
val cut_file : Filepath.t -> ((Filepath.t * Filepath.t) option, string) result
(** [cut_file p] is [Some (file, q)] with [file] an existing regular
    ...
    is [None]. In this case [p/] is [Some (p, Filepath.root p].
    The syntactic directoryness of [p] is kept in [q]. *)

val path_in_mount_to_image_path :
  image:Filepath.t -> mount_root:Filepath.t -> Filepath.t -> Filepath.t
...
val get_existing_path_in_mount :
  image:Filepath.t -> mount_root:Filepath.t -> Filepath.t ->
