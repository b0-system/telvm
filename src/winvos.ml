...
let virtio_win_iso =
  ...
  in
  Data_file.make Winvos ~filename ~src:(Url url)
...
let arm64_data =
  ...
  in
  { arch = Os.Arch.arm64;
    iso = Data_file.make Winvos ~filename:"winvos-arm64.iso" ~src:(Url iso_url);
    powershell_zip =
      Data_file.make Winvos ~filename:"powershell-arm64.zip"
        ~src:(Url pwsh_url) }
...
let x86_64_data =
  ...
  { arch = Os.Arch.x86_64;
    iso =
      Data_file.make Winvos ~filename:"winvos-x86_64.iso" ~src:(Url iso_url);
    powershell_zip =
      Data_file.make Winvos ~filename:"powershell-x86_64.zip"
        ~src:(Url pwsh_url) }
...
let get_iso_file ~data_dir ~arch =
  let* iso = iso ~arch in
  Data_file.get ~data_dir iso
...
let get_powershell_zip_file ~data_dir ~arch =
  let* powershell_zip = powershell_zip ~arch in
  Data_file.get ~data_dir powershell_zip

let ensure_data ~data_dir ~archs =
  let get_data_file ~data_dir data_file =
    let* httpc = B0_http.Http_client.make ~progress:true () in
    Data_file.ensure ~available_log:Log.Info ~httpc ~data_dir data_file
  in
  let get_winvos_iso arch =
    let* winvos_iso = iso ~arch in
    let* winvos_iso = get_data_file ~data_dir winvos_iso in
    Ok ()
  in
  let* _virtio_win_iso = get_data_file ~data_dir virtio_win_iso in
  let archs = List.distinct Os.Arch.compare archs in
  let* () = List.iter_stop_on_error get_winvos_iso archs in
  Ok ()
...
module Pkg = struct
  ...
  let list ~data_dir ~arch ~names ~add_matching_wow64 =
    let* image = get_iso_file ~data_dir ~arch in
    Image.with_mount ~image @@ fun ~mount_root ->
...
module Imager = struct
  ...
  let make_imager_blank_image ~force ~make_path ~byte_size img =
    let byte_size = Image.byte_size_gb_multiple byte_size in
    (* N.B. The windows bootloader fails with Gpt and/or ExFat (at least
       on macOS) formatted disks *)
    let partition_scheme = Image.Mbr and file_system = Image.Fat32 in
    let name = None in
    Image.make
  ...
  let make_bootstrap_image
    ...
    let with_imager_winvos_iso_mount f =
      ...
      else Image.with_mount ~image:imager_winvos_iso f
    in
    let* () =
      let* winvos_stat = Os.Path.stat imager_winvos_iso in
      let byte_size = winvos_stat.Unix.st_size * 2 in
      let byte_size = Image.byte_size_gb_multiple byte_size in
      let partition_scheme = Image.Gpt and file_system = Image.Exfat in
      let name = None and make_path = false in
      Image.make
        ~partition_scheme ~file_system ~byte_size ~name ~force ~make_path img
    in
    Image.with_mount ~image:img @@ fun ~mount_root:img_mount ->
    let* () =
  ...
  let run_bootstrap
      ~mem_size ~build_script_src ~bootstrap_arch ~bootstrap_winvos_iso
      ~imager_winvos_iso ~virtio_win_iso ~dst
    =
    log_bootstrap ();
    Adhoc.with_tmp_file ~ext:".vhdx" @@ fun validation_os_vhdx ->
    Adhoc.with_tmp_file ~ext:".img" @@ fun bootstrap_img ->
    let* () =
      Image.with_mount ~image:bootstrap_winvos_iso @@
    ...
    let* qemu =
      ...
      in
      Qemu.cmd
        ~guest_arch:bootstrap_arch ~mem_size ~drives ~use_virtio:false
  ...
  | None ->
      let file = Fmt.str "imager-%a.img" Os.Arch.pp imager_arch in
      Data_dir.resolve_path ~data_dir Fpath.(v "@winvos" / file)
  ...
  let make
    ...
    let archs = [bootstrap_arch; imager_arch] in
    let* () = ensure_data ~data_dir ~archs in
    let* virtio_win_iso = Data_file.get ~data_dir virtio_win_iso in
    let* bootstrap_winvos_iso = get_iso_file ~data_dir ~arch:bootstrap_arch in
    let* imager_winvos_iso = get_iso_file ~data_dir ~arch:imager_arch in
    let* () =
      let* winvos_stat = Os.Path.stat imager_winvos_iso in
  ...
  let get ~data_dir ~imager_arch file =
    ...
    | None ->
        ...
        let* () =
          ...
          in
          let mem_size = None in
          make
            ~mem_size ~data_dir ~build_script_src ~bootstrap_arch ~imager_arch
            ~force:false ~make_path:true ~dst:(Some imager)
        in
        Ok (imager, imager_arch)
end
