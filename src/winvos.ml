...
let copy_winvos_vhdx ~force ~make_path ~winvos_mount ~dst =
  let src = Filepath.(winvos_mount / "ValidationOS.vhdx") in
...
| Ok None as v -> v
| Ok Some file ->
    let* url = Os.File.read Filepath.(file + ".url") in
    Result.map Option.some (parse_version_from_url url)

let upstream_version ~arch =
  let* httpc = B0_http.Client.make ~progress:false () in
  let* data = iso ~arch in
  let Url url = Data_file.src data in
  let* final_url = B0_http.Client.head_request_follow_location httpc url in
...
module Pkg = struct
  ...
  let pkg_list ~mount_root ~names ~add_matching_wow64 =
    ...
    let* cab_files =
      ...
      and recurse = true in
      Os.Dir.contents
        ~kind ~rel ~dotfiles ~follow_symlinks ~recurse Filepath.(mount_root / cabs)
    in
    let cab_files = List.map (Filepath.append (Filepath.v cabs)) cab_files in
  ...
  let contents ~data_dir ~arch ~names ~add_matching_wow64 =
    ...
    let abs f = Filepath.append mount_root f in
...
module Imager = struct
  ...
  let make_bootstrap_image
    ...
    let with_imager_winvos_iso_mount f =
      ...
      if Filepath.equal bootstrap_winvos_iso imager_winvos_iso
    ...
    let* () =
      ...
      let cab =
        Filepath.v "cabs/neutral/Microsoft-WinVOS-Provisioning-Package.cab"
      in
      let cab = Filepath.(bootstrap_winvos_mount // cab) in
    ...
    let* () =
      ...
      let force = true and make_path = false in
      let cmd = Filepath.(img_mount / "bootstrap.cmd") in
  ...
  let make
    ...
    let warn_exists dst =
      ...
      m "@[<v>Skipping bootstrap, image %a exists@,Use %a to overwrite@]"
        Filepath.pp dst Fmt.code "--force"
    in
  ...
  let get ~data_dir ~imager_arch file =
    ...
    | None ->
        let imager_arch = Os.Arch.get () in
