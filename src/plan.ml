...
let parse_create doc =
  let* copy_dirs =
    ...
    loop [] copy_dirs
  in
  let* users = parse_users (B0_fini.find_section ["user"] doc) in
  let* winvos = parse_winvos (B0_fini.find_section ["winvos"] doc) in
...
let of_ini ?(file = Fpath.dash) src =
  ...
  in
  let* create = parse_create (B0_fini.find_section ["create"] doc) in
