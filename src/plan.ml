...
let of_ini ?(file = Filepath.dash) src =
  let basename = Filepath.basename ~drop_exts:true file in
  let basename = if basename = "-" then "winvos" else basename in
  let arch = None in
  let* doc = B0_fini.of_string ~file:(Filepath.to_string file) src in
...
let dism_add_package pkgs =
  ...
  let package_path_arg cabfile =
    let to_win_path f = (* TODO Filepath *)
      String.concat "\\" ("" :: Filepath.to_segments f)
    in
