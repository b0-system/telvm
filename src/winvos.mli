...
(** Windows Validation OS (WinVOS) munging *)
...
val find_iso_file :
  data_dir:Fpath.t -> arch:Os.Arch.t -> (Fpath.t option, string) result
...
val get_powershell_zip_file :
  data_dir:Fpath.t -> arch:Os.Arch.t -> (Fpath.t, string) result

val ensure_data :
  data_dir:B0_std.Fpath.t ->
  archs:B0_std.Os.Arch.t list -> (unit, string) result
(** [enusure_data ~data_dir ~archs] ensure we have data for [archs]
    and [winvos]. *)
...
module Imager : sig
  ...
  val make :
    data_dir:Fpath.t -> mem_size:string option -> bootstrap_arch:Os.Arch.t ->
    build_script_src:string -> imager_arch:Os.Arch.t ->
    force:bool -> make_path:bool -> dst:Fpath.t option ->
    (unit, string) result
