...
val get_or_download_virtio_win_iso :
  data_dir:Filepath.t -> (Filepath.t, string) result

val get_or_download_iso :
  data_dir:Filepath.t -> arch:Os.Arch.t -> (Filepath.t, string) result
...
val installed_version :
  data_dir:Filepath.t -> arch:Os.Arch.t -> (string option, string) result
...
val cab_extract_file :
  force:bool -> make_path:bool -> cab:Filepath.t -> file:string ->
  dst_dir:Filepath.t -> (unit, string) result
...
module Pkg : sig
  ...
  val contents :
    data_dir:Filepath.t -> arch:Os.Arch.t -> names:string list ->
...
val copy_winvos_vhdx :
  force:bool -> make_path:bool -> winvos_mount:Filepath.t ->
  dst:Filepath.t -> (unit, string) result
...
module Imager : sig
  ...
  val get_build_script_src :
    data_dir:Filepath.t -> bootstrap_arch:Os.Arch.t -> imager_arch:Os.Arch.t ->
    Filepath.t option -> (string, string) result
  ...
  val get :
    data_dir:Filepath.t -> imager_arch:Os.Arch.t option ->
    Filepath.t option -> (Filepath.t * Os.Arch.t, string) result
