...
let find_firmware_json ~host_os ~guest_arch =
  ...
  let clean dir =
    ...
    Result.map Option.some (Filepath.of_string (String.trim dir))
  in
  let dirs = List.filter_map clean (String.split_all ~sep:"\n" dirs) in
  let rec loop = function
  | [] ->
      Fmt.error "@[<v>Could not find firmware .json file %s in directories:@,%a"
        firmware_json_filename Fmt.(list Filepath.pp) dirs
  | dir :: dirs ->
      let file =
        Log.if_error ~use:None @@
        Os.Path.exists_realpath
          Filepath.(dir / "firmware" / firmware_json_filename)
      in
      match file with
      | Some file -> Ok file
      | None -> loop dirs
...
let firmware_executable_of_json file =
  ...
  let* json = Json.of_string ~file:(Filepath.to_string file) json in
  let* exe, format = Jsonq.query query json in
  let* exe = Filepath.of_string exe in
  Ok (exe, format)
...
let uefi_drive ~uefi_code:(code, format) =
  let code = Filepath.to_string code in
...
let drive_device_of_path path =
  match String.split_last ~sep:"@" (Filepath.to_string path) with
  | None -> Ok (None, path)
  | Some ("", p) -> Ok (None, path)
  | Some (p, device) ->
      let is_perhaps_windows_drive = String.length p = 1 in
      match drive_device_of_string device with
      | Ok device -> Ok (Some device, Filepath.v p)
      | Error e when is_perhaps_windows_drive -> Ok (None, path)
      | Error e -> Fmt.error "%a: @[%a@]" Filepath.pp path Fmt.styled_text e

type drive = drive_device * Filepath.t
...
let add_drive_cmd id ?bus ?serial ?(cache = "") ~image ~driver () =
  ...
  let drive =
    ...
    let format =
      if Filepath.has_ext ".img" image || Filepath.has_ext ".iso" image
      then "format=raw," else ""
    in
    Fmt.str "if=none,id=%s,%s%sfile=%a" driveid cache format Filepath.pp image
  in
  Cmd.(arg "-drive" % drive %
...
let add_cdrom_drive_cmd id ~image ~driver =
  ...
  let drive =
    Fmt.str
      "if=none,id=%s,media=cdrom,format=raw,file=%a" driveid Filepath.pp image
  in
  Cmd.(arg "-drive" % drive %
...
let add_virtio_9p_pci_cmd id ~dir =
  ...
  let devid = Fmt.str "dev%02d" id in
  let mount_tag = Filepath.basename dir in
  let fsdev =
    Fmt.str
      "local,security_model=mapped-xattr,id=%s,path=%a" fsid Filepath.pp dir
  in
  let device =
...
let add_smb_cmd id ~dir =
  let devid = Fmt.str "dev%02d" id in
  let nic =
    Fmt.str "user,id=%s,smb=%a" devid Filepath.pp dir
  in
  Cmd.(arg "-nic" % nic)
...
let cmd
    ...
    ~use_usb_input
  =
  let host_os = Os.Name.get () in
  let host_arch = Os.Arch.get () in
  let* base =
