...
let handle_fs ~force ~make_path file =
  ...
  if exists && not force then Filepath.error file "File exists" else
  let parent_dir = Filepath.parent file in
  (* handle [make_path] *)
  ...
  else Filepath.error parent_dir "No such directory"
...
let volname ~name ~file_system file =
  ...
  | Some name -> name | None -> Filepath.basename ~drop_exts:true file
  in
...
let make_blank_image ~force ~make_path ~byte_size file =
  ...
  | Unix.Unix_error (e, _, _)  -> Filepath.error file "%s" (Unix.error_message e)

let err_mount_failure ~image e =
  Fmt.str "@[<v>Failed to mount image %a:@,%s@]" Filepath.pp image e
...
module Linux = struct
  ...
  let make
    ...
    | Mbr | Gpt ->
        ...
        let* dev = Filepath.of_string dev in
...
let with_mount' ~image f = match Os.Name.get () with
...
let cut_file p =
  try
    ...
    let rec loop suffix p =
      if Filepath.is_root p || Filepath.is_current_dir p then Ok None else
      if exists p then
        let suffix = match suffix with
        | [] | [""] -> [""; ""]
        | suffix -> "" :: suffix
        in
        Ok (Some (p, Filepath.of_segments suffix))
      else
      let up, seg = Filepath.cut_last_segment p in
      loop (seg :: suffix) up
    in
    loop [] p
  with
...
let path_in_mount_to_image_path ~image ~mount_root p =
  match Filepath.drop_strict_prefix ~prefix:mount_root p with
  | None -> assert false | Some p -> Filepath.(image // p)

let rec get_existing_path_in_mount ~image ~mount_root path =
  let* real_mount_root = Os.Path.realpath mount_root in
  let rel = Filepath.drop_root_sep path in
  let path_in_real_mount = Filepath.(real_mount_root // rel) in
  let* real =
    try
      Filepath.of_string (Unix.realpath (Filepath.to_string path_in_real_mount))
    with
    | Unix.Unix_error (EINTR, _, _) ->
        get_existing_path_in_mount ~image ~mount_root rel
    | Unix.Unix_error ((ENOENT|ENOTDIR), _, _) ->
        Fmt.error "%a: No such path" Filepath.pp Filepath.(image // rel)
    | Unix.Unix_error (e, _, _) ->
        Fmt.error "%a: %s"
          Filepath.pp Filepath.(image // rel) (Unix.error_message e)
  in
  if Filepath.strictly_starts_with ~prefix:real_mount_root real
  then Ok Filepath.(mount_root // rel) else
  if Filepath.equal real_mount_root real
  then Ok mount_root
  else Fmt.error "Path %a points outside the disk image mount" Filepath.pp rel
...
let get_syntactic_path_in_mount ~image ~mount_root path =
  ...
  let rel = Filepath.drop_root_sep path in
  let* real_mount_root = Os.Path.realpath mount_root in
  let path = Filepath.try_drop_relative_dirs Filepath.(real_mount_root // rel) in
  if Filepath.strictly_starts_with ~prefix:real_mount_root path
  then Ok Filepath.(mount_root // rel)
  else if Filepath.equal real_mount_root path
  then Ok mount_root
  else Fmt.error "Path %a points outside the disk image mount" Filepath.pp rel
